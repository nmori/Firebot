            ...
            template: `
                ...
                >
                    <div style="flex-grow: 1;">
                        <div style="font-weight: 900;">{{$ctrl.macroDisplay()}} <i class="fal fa-plus-circle clickable" uib-tooltip="テキスト欄に追加" style="color: #0b8dc6" ng-click="$ctrl.onAddToTextClicked()"></i></div>
                        <div class="muted">{{$ctrl.macro.description}}</div>
                    </div>
                    <div style="width: 50px;" class="flex justify-end items-center">
                        <button ng-show="$ctrl.isHovering" class="edit-macro-btn" ng-click="$ctrl.onEditClicked()">編集</button>
                    </div>
                </div>
                `,
            controller: function() {
