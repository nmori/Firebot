            ...
            template: `
                ...
                <div class="modal-header sticky-header edit-widget-header">
                    ...
                    <firebot-button
                        ...
                        tooltip="{{ $ctrl.widget.type == null ? '種別を選択するとライブプレビューが有効になります' : (!$ctrl.typeSupportsLivePreview ? 'このウィジェット種別はライブプレビューに対応していません' : '有効にすると、オーバーレイの変更がリアルタイムで確認できます') }}"
                ...
                <div class="modal-body">
                    <form name="widgetSettings">
                        ...
                        <div class="form-group" ng-class="{'has-error': $ctrl.formFieldHasError('type')}">
                            <label for="expression" class="control-label">種別</label>
                            <firebot-searchable-select
                                name="type"
                                ng-model="$ctrl.widget.type"
                                placeholder="種別を選択または検索..."
                            ...
                            <div ng-if="$ctrl.livePreviewActive && $ctrl.isNewWidget" class="help-block">ライブプレビューが有効な間は種別を変更できません。</div>
                        </div>
                        ...
                        <div ng-if="$ctrl.widget.type && $ctrl.selectedType != null">
                            ...
                            <div class="form-group" ng-class="{'has-error': $ctrl.formFieldHasError('name')}">
                                ...
                                <div style="position: relative;">
                                    <input
                                        ...
                                        placeholder="{{ $ctrl.selectedType.name }}に名前を付けてください"
                                ...
                                <div ng-if="$ctrl.formFieldHasError('name')">
                                    ...
                                    <span ng-if="widgetSettings.name.$error.minlength" class="help-block">名前は1文字以上である必要があります。</span>
                                    <span ng-if="widgetSettings.name.$error.valid && !widgetSettings.name.$error.required && !widgetSettings.name.$error.minlength" class="help-block">無効な名前形式です。</span>
                                    <span ng-if="widgetSettings.name.$error.taken" class="help-block">この名前は既に使用されています。</span>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="fb-select" id="overlay-instance" ng-model="$ctrl.widget.overlayInstance">
                                    <option label="既定" value="">既定</option>
                            ...
                            <div class="form-group" ng-if="$ctrl.userCanConfigure.zIndex">
                                <label for="z-index" class="control-label">z-index <tooltip text="'要素が重なった場合にどれを前面に表示するかを制御します。数値が大きいほど前面に表示されます。'"></tooltip></label>
                                <input
                                    type="number"
                                    id="z-index"
                                    name="z-index"
                                ...
                                />
                                <span class="help-block">省略可能。</span>
                            </div>
                            ...
                            <div ng-if="$ctrl.selectedType != null && $ctrl.selectedType.settingsSchema != null">
                                ...
                                <div style="margin-top: 0.25rem;font-size: 14px; color: #99a1af; margin-bottom: 1.25rem;">このウィジェットの設定を以下で行ってください。</div>
                                <dynamic-parameter
            ...
            controller: function($scope, modalFactory, overlayWidgetsService, settingsService, ngToast, backendCommunicator, utilityService) {
                ...
                $ctrl.save = () => {
                    ...
                    if (successful) {
                        ...
                        ngToast.create("オーバーレイウィジェットの保存に失敗しました。もう一度お試しいただくか、ログで詳細をご確認ください。");
                    }
                };
                ...
                $ctrl.delete = function() {
                    ...
                    modalFactory
                        .showConfirmationModal({
                            ...
                            question: `このオーバーレイウィジェットを削除してもよろしいですか？`,
                            confirmLabel: "削除",
