        ...
        .component("rewardRedemption", {
            ...
            template: `
                <div class="reward-redemption" ng-class="{ isHighlight: $ctrl.redemption.id === 'highlight-message' }">
                    ...
                    <b>{{$ctrl.redemption.user.displayName}}{{($ctrl.redemption.user.displayName.toLowerCase() !== $ctrl.redemption.user.username.toLowerCase() ? " (" + $ctrl.redemption.user.username + ")" : "")}}</b> <span>が引き換え</span> <b>{{$ctrl.redemption.reward.name}}</b>
                </div>
            `,
            controller: function() {
                const $ctrl = this;
