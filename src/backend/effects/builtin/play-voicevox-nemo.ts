...
const model: EffectType<{
    ...
    definition: {
        id: "firebot:playvoicevox-nemo",
        name: "VOICEVOX Nemoで読み上げ",
        description: "VOICEVOX Nemoを使って音声読み上げを行います",
    ...
    optionsController: async ($scope) => {
        ...
        if ($scope.effect.voicevoxPort == null) {
            $scope.effect.voicevoxPort = 50121;
        }
        ...
        try {
            ...
            const responseData: Array<{ name: string; styles: Array<{ name: string; id: number }> }> =
    ...
    onTriggerEvent: async (event) => {
        ...
        try {
            ...
            const queryData = await queryRes.text();
