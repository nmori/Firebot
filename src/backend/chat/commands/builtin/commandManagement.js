        ...
        return new Promise(async (resolve) => {
            ...
            switch (triggeredArg) {
                ...
                case "cooldown": {
                    ...
                    await chat.sendChatMessage(
                        `'${trigger}' の実行まち設定を更新しました: ${userCooldown}秒 (ユーザ), ${globalCooldown}秒 (全体)`
                    );                    
                ...
                case "restrict": {
                    if (args.length < 3 || remainingData === "") {
                        await chat.sendChatMessage(
                            `無効な命令です. Usage: ${event.command.trigger} ${usage}`
                        );
                        return resolve();
                    }
                    ...
                    if (command == null) {
                        await chat.sendChatMessage(
                            `次のトリガーは見つかりません '${trigger}', please try again.`
                        );
                        return resolve();
                    }
                    ...
                    await chat.sendChatMessage(`'${trigger}' の制限を設定しました: ${remainingData}`);
                ...
                case "remove": {
                    ...
                    await chat.sendChatMessage(`コマンドから '${trigger}'を削除しました.`);
                    break;
                }
                case "disable":
                case "enable": {
                ...
                case "addalias": {
                    ...
                    if (args.length < 3 || alias === "") {
                        await chat.sendChatMessage(
                            `無効なコマンドです. Usage: ${event.command.trigger} ${usage}`
                        );
                        return resolve();
                    }
                ...
                const command = commandManager.getAllCustomCommands().find(c => c.trigger === trigger);
                    ...
                    if (aliasIndex > -1) {
                        await chat.sendChatMessage(
                            `'${trigger}'を持つコマンドとして'${alias}'が既に存在します。 .`
                        );
                        return resolve();
                    }
