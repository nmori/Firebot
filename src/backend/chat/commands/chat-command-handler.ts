...
class CommandHandler {
    ...
    async handleChatMessage(firebotChatMessage: FirebotChatMessage): Promise<{
        ...
        if (userCmd.isInvalidSubcommandTrigger === true) {
            await TwitchApi.chat.sendChatMessage(`無効なコマンド： 不明な引数が使用されました。`, null, true);
            return result;
        }
        ...
        if (userCmd.args.length < minArgs) {
            ...
            await TwitchApi.chat.sendChatMessage(`無効なコマンドです。使用法： ${command.trigger} ${usage || ""}`, null, true);
            return result;
        }
