...
type WatchStreakChatNotificationEvent = {
    type: "watch_streak";
    chatterName: string;
    chatterId: string;
    ...
    private createSubscriptions(): void {
        ...
        const bitsSubscription = this._eventSubListener.onChannelBitsUse(streamer.userId, async (event) => {
            switch (event.type) {
                case "cheer": {
                    const totalBits =
                        (await TwitchApi.bits.getChannelBitsLeaderboard(1, "all", new Date(), event.userId))[0]
                            ?.amount ?? 0;
        ...
        const autoModMessageHoldSub = this._eventSubListener.onAutoModMessageHoldV2(
            streamer.userId,
            streamer.userId,
            async (event) => {
        ...
        const customRewardUpdateSubscription = this._eventSubListener.onChannelRewardUpdate(
            streamer.userId,
            (event) => {
                channelRewardManager.saveTwitchDataForChannelReward(mapEventSubRewardToTwitchData(event));
            }
        ...
        const customRewardRemoveSubscription = this._eventSubListener.onChannelRewardRemove(
            streamer.userId,
            async (event) => {
        ...
        // Shared Chat Started
        const sharedChatStartedSubscription = this._eventSubListener.onChannelSharedChatSessionBegin(
            streamer.userId,
            async (event) => {
        ...
        // Shared Chat Updated
        const sharedChatUpdatedSubscription = this._eventSubListener.onChannelSharedChatSessionUpdate(
            streamer.userId,
            async (event) => {
                SharedChatCache.participants = await mapSharedChatParticipants(event.participants);
        ...
        // Shared Chat Ended
        const sharedChatEndedSubscription = this._eventSubListener.onChannelSharedChatSessionEnd(
            streamer.userId,
            () => {
        ...
        // Hype Train start
        const hypeTrainBeginSubscription = this._eventSubListener.onChannelHypeTrainBeginV2(
            streamer.userId,
            (event) => {
        ...
        // Hype Train progress
        const hypeTrainProgressSubscription = this._eventSubListener.onChannelHypeTrainProgressV2(
            streamer.userId,
            (event) => {
        ...
        const channelGoalProgressSubscription = this._eventSubListener.onChannelGoalProgress(
            streamer.userId,
            (event) => {
        ...
        const predictionBeginSubscription = this._eventSubListener.onChannelPredictionBegin(
            streamer.userId,
            (event) => {
        ...
        const predictionProgressSubscription = this._eventSubListener.onChannelPredictionProgress(
            streamer.userId,
            (event) => {
        ...
        const charityCampaignStartSubscription = this._eventSubListener.onChannelCharityCampaignStart(
            streamer.userId,
            (event) => {
        ...
        // Charity Donation
        const charityDonationSubscription = this._eventSubListener.onChannelCharityDonation(
            streamer.userId,
            (event) => {
        ...
        const charityCampaignProgressSubscription = this._eventSubListener.onChannelCharityCampaignProgress(
            streamer.userId,
            (event) => {
        ...
        const charityCampaignEndSubscription = this._eventSubListener.onChannelCharityCampaignStop(
            streamer.userId,
            (event) => {
        ...
        // Chat notification
        const chatNotificationSubscription = this._eventSubListener.onChannelChatNotification(
            streamer.userId,
            streamer.userId,
            async (event) => {
                if ((event as { type: string }).type === "watch_streak") {
                    const watchStreakEvent = event as unknown as WatchStreakChatNotificationEvent;

                    TwitchEventHandlers.watchStreak.triggerWatchStreak(
                        watchStreakEvent.chatterName,
                        watchStreakEvent.chatterId,
                        watchStreakEvent.chatterDisplayName,
                        watchStreakEvent.streakCount,
                        watchStreakEvent.channelPointsAwarded,
                        watchStreakEvent.messageText ?? ""
                    );

                    if (!!watchStreakEvent.messageText?.length) {
                        FirebotFrontendChatHelpers.sendChatMessageToFrontend(
                            await TwitchEventSubChatHelpers.buildChatMessageFromChatEvent(event as never)
                        );
                    }
                    ...
                    case "gift_paid_upgrade":
                        {
                            ...
                            const upgradeTier = (await (await event.getBroadcaster()).getSubscriber(event.chatterId))
                                .tier;
